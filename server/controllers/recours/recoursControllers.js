...
const getDecisions = async(req, res)=>{
  ...
  const sql= "SELECT decisions.*, decisions.date AS date_decision, reunions.date AS date_reunion FROM decisions JOIN reunions ON decisions.id_reunion = reunions.id_reunion ORDER BY decisions.id_decision ASC" ;
  db.query(sql, (err, result)=>{
      if(err)
          return res.json({statut: "erreur", message: "Une erreur est survenue"});
      else
