...
if(i===2 &&propertyValues[i]?.length >= 1){
  ...
  values.push(v);
    ...
    db.query(sql,[values],(err,result)=>{
      ...
      return res.json({success:"Recours ajoute avec succes", numDossier: req.app.locals.path.id_recours})
       })
}
...
const getRecours = async(req, res)=>{
    ...
    const sql='select * from recours where id_assure = ?';
...
const verfication=async(req,res)=>{
        ...
        db.query(sql,values,(err,results)=>{
            if (err) {
              ...
              if (results.length > 0) {
                ...
                return res.status(200).json({ message: "La décision n'existe pas" });
              }
          })
    }else if(+type===2){
 ...
 //console.log(result)
                ...
                return res.status(200).json({ message: 'Le recours a été déja traité par la commission locale' });
            
              } else {
                return res.status(200).json({ message: "Aucun recours n'est effectué en cette date" });
              }
             })
    }
}
...
const getDecisions = async(req, res)=>{
  ...
  const sql= "SELECT decisions.*, decisions.date AS date_decision, reunions.date AS date_reunion FROM decisions JOIN reunions ON decisions.id_reunion = reunions.id_reunion where decisions.id_assure = ? ORDER BY decisions.id_decision ASC" ;
  db.query(sql,[req.session?.userinfos?.id_assure], (err, result)=>{
      if(err)
          return res.json({statut: "erreur", message: "Une erreur est survenue"});
      else
